...
async function updateStatus(user: User, jobId: string, status: string) {
  const token = await user.getIdToken();
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[390px] flex-col bg-white shadow-none md:min-h-[780px] md:overflow-hidden md:rounded-[28px] md:shadow-2xl md:ring-1 md:ring-slate-200">
        <div className="flex-1 px-5 pb-6 pt-5">
          ...
          <section className="mt-8">
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-red-500">Customer jobs</p>
            ...
            {jobs.map((job) => (
              <article key={job.jobId} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.12em] text-red-500">{job.jobId}</p>
                    <h2 className="mt-1 text-lg font-bold">{job.selectedServiceCategory || "Service request"}</h2>
                  </div>
                  <span className="rounded-full border border-slate-200 px-3 py-1 text-xs font-bold capitalize text-slate-700">{job.status}</span>
                </div>
                ...
                <div className="mt-4 grid gap-2">
                  <button type="button" onClick={() => handleMessage(job)} disabled={activeJobId === job.jobId} className="flex h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 text-sm font-bold">
                    <MessageCircle className="h-4 w-4" /> Message contractor
                  </button>
                  {job.status === "in_progress" ? (
                    <button type="button" onClick={() => handleStatus(job.jobId, "completed")} disabled={activeJobId === job.jobId} className="flex h-12 items-center justify-center rounded-xl bg-red-500 text-sm font-bold text-white">Mark completed</button>
                  ) : null}
                  {job.status === "hired" ? (
                    <button type="button" onClick={() => handleStatus(job.jobId, "cancelled")} disabled={activeJobId === job.jobId} className="flex h-12 items-center justify-center rounded-xl bg-red-50 text-sm font-bold text-red-600">Cancel job</button>
                  ) : null}
                  {job.status === "completed" ? (
                    <Link href={`/customer/jobs/${encodeURIComponent(job.jobId)}/review`} className="flex h-12 items-center justify-center rounded-xl bg-red-500 text-sm font-bold text-white">Review contractor</Link>
                  ) : null}
                </div>
              </article>
            ))}
